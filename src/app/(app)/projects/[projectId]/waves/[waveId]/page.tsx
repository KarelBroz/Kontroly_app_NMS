...
export default async function WaveDetailPage({
  ...
  const totalVisitCount = await prisma.visit.count({ where: { waveId: wave.id } });
  const totalOpenFindingsCount = await prisma.finding.count({
    where: { status: FindingStatus.OPEN, visit: { waveId: wave.id } },
  });
  const reviewerOptions = await prisma.reviewer.findMany({
  ...
  let statusFiltered: typeof filteredVisits;
  if (statusFilter === "error") {
    statusFiltered = errorVisits;
  } else if (statusFilter === "clean") {
    statusFiltered = cleanVisits;
  } else if (statusFilter === "empty") {
    statusFiltered = emptyVisits;
  } else {
    statusFiltered = [...errorVisits, ...cleanVisits, ...emptyVisits];
  }

  const sortMode = searchParams.sort && searchParams.sort !== "priority" ? searchParams.sort : "priority";
  let bucketed: typeof filteredVisits;
  if (sortMode === "priority") {
    bucketed = statusFiltered;
  } else {
          ...
          <Card className="border-l-4 border-l-brand-green-400 p-0">
            ...
            <div className="border-b border-slate-100 bg-slate-50/60 px-6 py-4">
              <form className="flex flex-wrap items-end gap-3" method="GET">
                ...
                <a
                  href={
                    `/projects/${wave.projectId}/waves/${wave.id}/export` +
